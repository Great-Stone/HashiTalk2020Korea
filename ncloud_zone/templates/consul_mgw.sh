...
#consul connect envoy -mesh-gateway -register -service 'gateway-primary' -address $PRIVATE_IP:8080 -wan-address $PUBLICIP:8080 -admin-bind 127.0.0.1:19005 -token=$CONSUL_MESH_TOKEN &
#consul connect envoy -expose-servers -mesh-gateway -register -service 'gateway-primary' -address $PRIVATE_IP:8080 -wan-address $PUBLICIP:8080 -admin-bind 127.0.0.1:19005 &
